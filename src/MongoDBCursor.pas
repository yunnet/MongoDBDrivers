...
type
  ...
  public
    constructor Create(ACollection: TMongoCollection;
                       AQuery: IBSONObject;
                       AFields: IBSONObject;
...
uses
  Math, SysUtils, MongoDecoder;

{ TMongoDBCursor }
constructor TMongoDBCursor.Create(ACollection: TMongoCollection;
                                  AQuery, AFields: IBSONObject;
                                  AConnector: IMongoConnector;
                                  AProvider: IMongoProvider
                                  );
begin
...
begin
  Result := TMongoDBCursor.Create(FCollection, FQuery, FFields, FConnector, FProvider);
  with Result do
  begin
    FBatchSize := FBatchSize;
    FLimit:= FLimit;
    FSkip:= FSkip;
    FOrderBy:= FOrderBy;
    FExplain := FExplain;
...
begin
  ...
  vResponse := FProvider.OpenQuery(FStream,
                                   FCollection.DBName,
                                   FCollection.CollectionName,
                                   vQuery,
                                   FFields,
                                   FSkip,
                                   ChooseBatchSize(FBatchSize, FLimit, 0)
                                   );
...
function TMongoDBCursor.Hint(AIndexName: AnsiString): IMongoDBCursor;
begin
  AssertCursorIsNotOpen;
